...
import { AlertTriangle, ArrowLeft, Building2, CalendarDays, ChevronLeft, ChevronRight, Edit, Fuel, Gauge, Filter, GripVertical, Images, LayoutGrid, Loader2, Plus, Power, Search, ShieldCheck, Tag, Trash2, Truck as TruckIcon, User, X } from 'lucide-react';
...
import type { FilterOption, Truck, TruckComplianceResponse, TruckFilters, TruckFormState, TruckListResponse } from './trucks/types';
import { ImagePreviewModal } from '../../components/ImagePreviewModal';
import { formatDateKey, formatExpiryMessage, getTruckExpiryInfo } from './trucks/truckCompliance';

const USER_PROFILE_KEY = 'eco_user_profile';
const MANAGER = 32;
const DIRECTOR = 64;

type TruckTableColumnId = 'license_plate' | 'payload' | 'assignment' | 'compliance' | 'fuel_consumption_limit' | 'status' | 'actions';
...
const truckTableHeaders: Array<{ id: TruckTableColumnId; label: string; className?: string; locked?: boolean }> = [
  ...
  { id: 'assignment', label: 'NCC / Bưu cục' },
  { id: 'compliance', label: 'Đăng kiểm / bảo hiểm', className: 'min-w-[230px]' },
...
const emptyForm: TruckFormState = { license_plate: '', payload: '', driver_id: '', fuel_consumption_limit: '', status: 'AVAILABLE', bks: '', hub_id: '', vendor_id: '', document_image_urls: [], registration_expiry_date: '', insurance_expiry_date: '' };
...
const formatVendor = (truck: Truck) => [truck.vendor?.code, truck.vendor?.name].filter(Boolean).join(' · ') || truck.nha_xe || (truck.vendor_id ? `NCC #${truck.vendor_id}` : 'Chưa gán NCC');
...
export default function AdminTrucksPage() {
  ...
  const [filters, setFilters] = useState<TruckFilters>({ keyword: '', status: [], driver_id: '', page: 1, limit: 10, ownership_type: internalOnly ? 'INTERNAL' : 'VENDOR', vendor_id: '' });
  const [trucks, setTrucks] = useState<Truck[]>([]);
  const [hubs, setHubs] = useState<Array<{ id: string | number; code?: string; name?: string }>>([]);
  const [vendors, setVendors] = useState<Array<{ id: string | number; code?: string | null; name?: string | null }>>([]);
  const [total, setTotal] = useState(0);
  const [compliance, setCompliance] = useState<TruckComplianceResponse | null>(null);
  ...
  const formInternalMode = internalOnly;
  const visibleColumnSet = useMemo(() => new Set(visibleColumns), [visibleColumns]);
  const availableTruckHeaders = useMemo(() => internalOnly ? truckTableHeaders : truckTableHeaders.filter(header => header.id !== 'compliance'), [internalOnly]);
  const orderedVisibleHeaders = useMemo(() => columnOrder.map(columnId => availableTruckHeaders.find(header => header.id === columnId)).filter((header): header is (typeof truckTableHeaders)[number] => Boolean(header && visibleColumnSet.has(header.id))), [availableTruckHeaders, columnOrder, visibleColumnSet]);
  ...
  const activeFilterCount = Number(Boolean(filters.keyword.trim())) + filters.status.length + Number(Boolean(filters.vendor_id));
  ...
  const vendorOptions = useMemo(() => [{ value: '', label: 'Tất cả NCC' }, ...vendors.map(vendor => ({ value: normalizeId(vendor.id), label: [vendor.code, vendor.name].filter(Boolean).join(' · ') || `NCC #${vendor.id}` }))], [vendors]);
  const filterPanelGroups = useMemo(() => [
    { id: 'status', title: 'Trạng thái', icon: Tag, options: statusOptions, value: filters.status, searchPlaceholder: 'Tìm trạng thái...', onChange: (value: string[]) => updateFilter('status', value) },
    ...(!internalOnly ? [{ id: 'vendor', title: 'Nhà cung cấp', icon: Building2, options: vendorOptions.filter(option => option.value), value: filters.vendor_id ? [filters.vendor_id] : [], searchPlaceholder: 'Tìm NCC...', onChange: (value: string[]) => updateFilter('vendor_id', value.at(-1) || '') }] : []),
  ], [filters.status, filters.vendor_id, internalOnly, vendorOptions]);

  useEffect(() => {
    if (internalOnly) void fetchHubs();
    else void fetchVendors();
  }, [internalOnly]);
  ...
  const clearFilters = () => setFilters(prev => ({ ...prev, keyword: '', status: [], driver_id: '', vendor_id: '', page: 1 }));
  ...
  async function fetchTrucks() {
    ...
    try {
      ...
      const [response, complianceResponse] = await Promise.all([
        apiRequest<TruckListResponse | Truck[]>(`/trucks?${params.toString()}`),
        internalOnly ? apiRequest<TruckComplianceResponse>('/trucks/compliance-alerts').catch(() => null) : Promise.resolve(null),
      ]);
    ...
    } catch (err) { setError(err instanceof ApiError ? err.message : 'Không thể tải danh sách xe.'); setTrucks([]); setTotal(0); setCompliance(null); }
  ...
  async function fetchVendors() {
    try {
      const response = await apiRequest<Array<{ id: string | number; code?: string | null; name?: string | null }> | { data?: Array<{ id: string | number; code?: string | null; name?: string | null }>; items?: Array<{ id: string | number; code?: string | null; name?: string | null }> }>('/vendors/active?limit=100');
      setVendors(Array.isArray(response) ? response : response.data || response.items || []);
    } catch { setActionError('Không thể tải danh sách nhà cung cấp.'); }
  }
  ...
  async function submitForm() {
    if (isDocumentUploading) {
      setActionError('Vui lòng chờ tải ảnh giấy tờ xe hoàn tất.');
      return;
    }
    if (formInternalMode && !formState.hub_id) {
      setActionError('Xe nội bộ phải được gán bưu cục hoạt động.');
      return;
    }
    if (!formInternalMode && !formState.vendor_id) {
      setActionError('BKS đối tác phải được gán nhà cung cấp (NCC).');
      return;
    }
  ...
  function renderTruckCell(columnId: TruckTableColumnId, truck: Truck) {
    switch (columnId) {
      ...
      case 'assignment': return <td key={columnId} className="px-4 py-3 border-r border-border text-[13px] font-medium">{isInternalTruck(truck) ? formatHub(truck) : formatVendor(truck)}</td>;
      case 'compliance': return <td key={columnId} className="min-w-[230px] px-4 py-3 border-r border-border"><div className="space-y-1.5"><TruckExpiryBadge label="Đăng kiểm" value={truck.registration_expiry_date} icon={CalendarDays} /><TruckExpiryBadge label="Bảo hiểm" value={truck.insurance_expiry_date} icon={ShieldCheck} /></div></td>;
  ...
  return (
    <div className="h-full min-h-0 flex flex-col gap-2">
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden flex-1 min-h-0 flex flex-col">
        <div className="border-b border-border bg-card p-3 shrink-0 space-y-3">
          ...
          <div className={clsx('flex flex-wrap items-center gap-2', canDelete && selectedBulkDeleteCount > 0 && 'hidden md:flex')}>
            ...
            <ColumnSettings columns={availableTruckHeaders} columnOrder={columnOrder} visibleColumns={visibleColumns} onToggle={toggleColumn} onReorder={reorderColumn} />
          ...
          <div className="hidden md:flex flex-wrap items-center gap-2">
            ...
            {!internalOnly && <FilterSelect value={filters.vendor_id || ''} options={vendorOptions} onValueChange={value => updateFilter('vendor_id', String(value))} placeholder="Nhà cung cấp" icon={Building2} className="w-[240px]" />}
          </div>
          {internalOnly && compliance && <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold"><span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-slate-700">Đã nhập đăng kiểm: {compliance.meta.registration_tracked}</span><span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-slate-700">Đã nhập bảo hiểm: {compliance.meta.insurance_tracked}</span><span className={clsx('rounded-full border px-2.5 py-1', compliance.meta.total_alerts ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-emerald-200 bg-emerald-50 text-emerald-700')}>Sắp/quá hạn 15 ngày: {compliance.meta.total_alerts}</span></div>}
        </div>
      ...
      <AddEditTruckDialog isOpen={isFormOpen} isClosing={isFormClosing} isEditMode={isEditMode} isSubmitting={isSubmitting || isDocumentUploading} onClose={closeForm} onSubmit={submitForm} formState={formState} setFormField={setFormField} statusOptions={statusOptions} internalOnly={formInternalMode} hubOptions={hubOptions} vendorOptions={vendorOptions.filter(option => option.value)} onUploadingChange={setIsDocumentUploading} />
...
function toTruckPayload(formState: TruckFormState, internalOnly = false) {
  return { ...stripEmpty({ license_plate: formState.license_plate.toUpperCase().trim(), bks: (formState.bks || formState.license_plate).toUpperCase().trim(), payload: Number(formState.payload || 0), fuel_consumption_limit: Number(formState.fuel_consumption_limit || 0), status: formState.status, ownership_type: internalOnly ? 'INTERNAL' : 'VENDOR', hub_id: internalOnly ? formState.hub_id : '', vendor_id: internalOnly ? '' : formState.vendor_id, document_image_urls: internalOnly ? formState.document_image_urls : [] }), ...(internalOnly ? { registration_expiry_date: formState.registration_expiry_date || null, insurance_expiry_date: formState.insurance_expiry_date || null } : {}) };
}
...
function toFormState(truck: Truck): TruckFormState { return { license_plate: formatPlate(truck), payload: truck.payload == null ? '' : String(truck.payload), driver_id: '', fuel_consumption_limit: truck.fuel_consumption_limit == null ? '' : String(truck.fuel_consumption_limit), status: truck.status || 'AVAILABLE', bks: truck.bks || formatPlate(truck), hub_id: normalizeId(truck.hub_id ?? truck.hub?.id), vendor_id: normalizeId(truck.vendor_id ?? truck.vendor?.id), document_image_urls: Array.isArray(truck.document_image_urls) ? truck.document_image_urls.slice(0, 10) : [], registration_expiry_date: truck.registration_expiry_date?.slice(0, 10) || '', insurance_expiry_date: truck.insurance_expiry_date?.slice(0, 10) || '' }; }
...
function formatTruckExpiryDetail(value?: string | null) { const info = getTruckExpiryInfo(value); return info.state === 'MISSING' || info.state === 'VALID' ? formatExpiryMessage(info) : `${formatDateKey(value)} · ${formatExpiryMessage(info)}`; }
function TruckExpiryBadge({ label, value, icon: Icon }: { label: string; value?: string | null; icon: typeof TruckIcon }) { const info = getTruckExpiryInfo(value); return <div className={clsx('flex items-center gap-2 rounded-lg border px-2 py-1 text-[11px] font-bold', info.state === 'EXPIRED' ? 'border-red-200 bg-red-50 text-red-700' : info.state === 'DUE_SOON' ? 'border-amber-200 bg-amber-50 text-amber-800' : info.state === 'VALID' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-50 text-slate-500')}><Icon size={13} className="shrink-0" /><span className="shrink-0">{label}:</span><span className="truncate">{formatTruckExpiryDetail(value)}</span></div>; }
...
function TruckMobileCard(props: TruckActionProps) { const { truck } = props; const selected = Boolean(props.isSelected); return <article onClick={() => props.openDetail(truck)} className={clsx('cursor-pointer rounded-2xl border bg-white p-4 shadow-sm transition-[border-color,box-shadow] duration-150', selected ? 'animate-truck-card-select border-primary ring-1 ring-primary shadow-primary/10' : 'border-border')}><div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-blue-200 bg-blue-50 text-primary"><TruckIcon size={20} /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h3 className="text-[15px] font-extrabold text-foreground">{formatPlate(truck)}</h3><p className="mt-1 text-[12px] font-medium text-muted-foreground">{isInternalTruck(truck) ? formatHub(truck) : formatVendor(truck)}</p></div><input type="checkbox" checked={selected} disabled={!props.canSelect} onClick={event => event.stopPropagation()} onChange={props.onToggleSelect} className="h-4 w-4 rounded border-border accent-primary disabled:opacity-40" /></div><div className="mt-2"><StatusBadge truck={truck} /></div></div></div><div className="mt-4 rounded-xl bg-muted/20 p-3 text-[13px] font-bold text-foreground">{truck.payload ?? 0} kg · {truck.fuel_consumption_limit ?? 0} L/100km</div>{isInternalTruck(truck) && <div className="mt-3 space-y-1.5"><TruckExpiryBadge label="Đăng kiểm" value={truck.registration_expiry_date} icon={CalendarDays} /><TruckExpiryBadge label="Bảo hiểm" value={truck.insurance_expiry_date} icon={ShieldCheck} /></div>}<div className="mt-3 border-t border-border pt-3"><TruckActions {...props} /></div></article>; }
