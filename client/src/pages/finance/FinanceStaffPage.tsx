...
type FormState = {
  ...
  overtime_hourly_rate: string; standard_work_days: string; opening_salary_debt: string; note: string;
};

const emptyForm = (): FormState => ({ employee_code: '', full_name: '', department_id: '', position: '', phone: '', email: '', identity_number: '', address: '', hire_date: '', employment_status: 'ACTIVE', hub_id: '', user_id: '', base_salary: '', meal_allowance: '', transport_allowance: '', other_allowance: '', overtime_hourly_rate: '', standard_work_days: '26', opening_salary_debt: '', note: '' });
...
export default function FinanceStaffPage() {
  const canViewCompensation = ((getStoredAuthUser()?.role_mask ?? 0) & 64) !== 0;
  ...
  const openEdit = (row: StaffRecord) => {
    setEditing(row);
    setForm({
      employee_code: row.employee_code || '', full_name: row.full_name || '', department_id: String(row.department_id || row.department_record?.id || ''), position: row.position || '', phone: row.phone || '', email: row.email || '', identity_number: row.identity_number || '', address: row.address || '', hire_date: row.hire_date?.slice(0, 10) || '', employment_status: row.employment_status || 'ACTIVE', hub_id: String(row.hub_id || row.hub?.id || ''), user_id: String(row.user_id || row.user?.id || ''), base_salary: formatAmountInputFromNumber(row.base_salary), meal_allowance: formatAmountInputFromNumber(row.meal_allowance), transport_allowance: formatAmountInputFromNumber(row.transport_allowance), other_allowance: formatAmountInputFromNumber(row.other_allowance), overtime_hourly_rate: formatAmountInputFromNumber(row.overtime_hourly_rate), standard_work_days: String(row.standard_work_days || 26), opening_salary_debt: formatAmountInputFromNumber(row.opening_salary_debt), note: row.note || '',
    });
    setError('');
    setFormOpen(true);
  };
  ...
  const submit = async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        user_id: form.user_id || null, standard_work_days: Math.max(1, Number(form.standard_work_days) || 26), note: form.note.trim() || null,
      };
      if (canViewCompensation) {
        Object.assign(body, {
          base_salary: parseAmountInput(form.base_salary), meal_allowance: parseAmountInput(form.meal_allowance),
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3">
      ...
      <div className="min-h-0 flex-1 overflow-auto rounded-lg border border-border bg-white shadow-sm">{loading ? <State loading text="Đang tải nhân sự..." /> : !filtered.length ? <State text="Chưa có hồ sơ nhân sự." /> : <table className="w-full min-w-[1200px] text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-xs uppercase text-muted-foreground"><tr>{['Mã NV', 'Họ tên', 'Bộ phận / chức danh', 'Liên hệ', 'Bưu cục', ...(canViewCompensation ? ['Lương cơ bản', 'Phụ cấp'] : []), 'Tài khoản', 'Trạng thái', 'Thao tác'].map((label) => <th key={label} className="border-b border-border px-3 py-3">{label}</th>)}</tr></thead><tbody>{filtered.map((row) => { const allowance = Number(row.meal_allowance || 0) + Number(row.transport_allowance || 0) + Number(row.other_allowance || 0); return <tr key={row.id} className="border-b border-border last:border-0"><td className="px-3 py-3 font-black text-primary">{row.employee_code}</td><td className="px-3 py-3"><p className="font-extrabold">{row.full_name}</p><p className="text-xs text-muted-foreground">{row.identity_number || '—'}</p></td><td className="px-3 py-3"><p className="font-bold">{row.department_record?.name || row.department || '—'}</p><p className="text-xs text-muted-foreground">{row.position}</p></td><td className="px-3 py-3"><p>{row.phone}</p><p className="text-xs text-muted-foreground">{row.email || '—'}</p></td><td className="px-3 py-3">{[row.hub?.code, row.hub?.name].filter(Boolean).join(' · ') || '—'}</td>{canViewCompensation && <><td className="px-3 py-3 font-bold">{formatMoney(row.base_salary)}</td><td className="px-3 py-3 font-bold">{formatMoney(allowance)}</td></>}<td className="px-3 py-3">{row.user ? row.user.full_name || row.user.username : 'Không dùng user'}</td><td className="px-3 py-3"><span className={`rounded-full border px-2 py-1 text-xs font-bold ${row.employment_status === 'ACTIVE' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-50 text-slate-500'}`}>{row.employment_status === 'ACTIVE' ? 'Đang làm' : 'Đã nghỉ'}</span></td><td className="px-3 py-3"><div className="flex gap-1"><button type="button" title="Sửa" onClick={() => openEdit(row)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-border text-primary"><Pencil size={14} /></button>{row.employment_status === 'ACTIVE' && <button type="button" title="Ngừng làm việc" onClick={() => void deactivate(row)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-red-200 text-red-600"><Trash2 size={14} /></button>}</div></td></tr>; })}</tbody></table>}</div>
      {formOpen && <StaffDialog form={form} editing={Boolean(editing)} canViewCompensation={canViewCompensation} departments={departments.filter((item) => item.is_active)} hubs={hubs} users={users} saving={saving} error={error} onChange={(patch) => setForm((current) => ({ ...current, ...patch }))} onClose={() => setFormOpen(false)} onSubmit={() => void submit()} />}
...
function StaffDialog({ form, departments, hubs, users, saving, error, onChange, onClose, onSubmit, editing, canViewCompensation }: { form: FormState; departments: StaffDepartment[]; hubs: Reference[]; users: Reference[]; saving: boolean; error: string; editing: boolean; canViewCompensation: boolean; onChange: (patch: Partial<FormState>) => void; onClose: () => void; onSubmit: () => void }) {
