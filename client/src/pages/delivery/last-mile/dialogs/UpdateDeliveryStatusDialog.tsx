          ...
          {currentStatus === 'AT_DEST_HUB' && !isCustomerPickup && resources && (
            <div className="space-y-3 rounded-xl border border-border bg-slate-50 p-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button type="button" onClick={() => { setAssignmentType('INTERNAL'); setUploadError(''); }} className={`h-9 rounded-lg border text-[12px] font-black ${assignmentType === 'INTERNAL' ? 'border-primary bg-blue-50 text-primary' : 'border-border bg-white text-muted-foreground'}`}>Xe nội bộ</button>
                <button type="button" onClick={() => { setAssignmentType('PARTNER'); setUploadError(''); }} className={`h-9 rounded-lg border text-[12px] font-black ${assignmentType === 'PARTNER' ? 'border-primary bg-blue-50 text-primary' : 'border-border bg-white text-muted-foreground'}`}>Đối tác</button>
                <button type="button" onClick={() => { setAssignmentType('TECHNOLOGY'); setUploadError(''); }} className={`h-9 rounded-lg border text-[12px] font-black ${assignmentType === 'TECHNOLOGY' ? 'border-primary bg-blue-50 text-primary' : 'border-border bg-white text-muted-foreground'}`}>Xe công nghệ</button>
              </div>
              {assignmentType === 'INTERNAL' ? (
                <div className="grid gap-2">
                  <select value={driverId} onChange={(event) => { const value = event.target.value; setDriverId(value); setUploadError(''); const driver = resources.drivers.find((item) => String(item.id) === value); if (driver) setDriverName(String(driver.name || driver.username || '')); }} className="h-10 rounded-lg border border-border bg-white px-3 text-[13px] font-bold text-foreground outline-none">
