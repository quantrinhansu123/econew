          ...
          {loading ? <StateBlock icon={<Loader2 className="animate-spin" size={24} />} title="Đang tải đơn phải thu" description="Hệ thống đang lấy các bill có tiền phải thu khi phát." />
            : error ? <StateBlock icon={<AlertTriangle size={24} />} title="Không tải được dữ liệu" description={error} />
              : !waybills.length ? <StateBlock icon={<WalletCards size={24} />} title="Không có đơn phù hợp" description="Không còn bill chờ xác nhận hoặc không có bill theo bộ lọc hiện tại." />
                : <>
                  <table className="hidden md:table w-full min-w-[1900px] text-left border-collapse">
                    <thead className="sticky top-0 z-10 bg-slate-100 text-[10px] uppercase tracking-wide text-slate-600"><tr>{['Đơn hàng', 'Mã KH', 'Chuyến', 'Mã BK', 'HUB đến / người thu', 'Người giữ COD', 'Ngày gửi', 'Ngày giao', 'Cước', 'Cước thu ĐN', 'COD', 'Phải xác nhận', 'Hình thức TT', 'Tình trạng đơn', 'Thu COD', 'Sổ quỹ', 'Thao tác'].map((header) => <th key={header} className="border-b border-r border-border px-3 py-2.5 font-extrabold last:border-r-0">{header}</th>)}</tr></thead>
                    <tbody>{waybills.map((waybill) => <CodTableRow key={String(waybill.id)} waybill={waybill} onBill={() => navigate(`/orders/new?edit=${encodeURIComponent(String(waybill.id))}`)} onManifest={() => waybill.manifest_id && navigate(`/warehouse/manifests?openManifestId=${encodeURIComponent(String(waybill.manifest_id))}`)} onConfirm={() => openConfirm(waybill)} />)}</tbody>
                  </table>
